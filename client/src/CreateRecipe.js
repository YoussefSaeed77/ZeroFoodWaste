...
const CreateRecipe = () => {
    ...
    const [formData, setFormData] = useState({userId: currentUser._id,vegetarian: false, vegan: false, glutenFree: false});
    const [step, setStep] = useState({stepNumber: '', step: '', equipment: ''});
    ...
    useEffect(() => {
        if (!(formData.title === undefined || formData.summary === undefined || formData.ingredients === undefined ||
            formData.servingNumber === undefined || formData.instructions.length === 0 || formData.readyInMinutes === undefined)) {
    ...
    return (
        <>
        {
            ...
            <InputDiv>
                ...
                <Input required id="title" type="text" onChange={(e) => handleChange(e.target.id, e.target.value)}></Input>
            </InputDiv>
            <DietDiv>
                <DietDiv2>
                    <Label htmlFor="vegetarian">Vegetarian</Label>
                    <InputDiet required id="vegetarian"  type="checkbox" onChange={(e) => {handleChange(e.target.id, !formData.vegetarian)}}></InputDiet>
                </DietDiv2>
                <DietDiv2>
                    <Label htmlFor="vegan">Vegan</Label>
                    <InputDiet required id="vegan"  type="checkbox" onChange={(e) => {handleChange(e.target.id, !formData.vegan)}}></InputDiet>
                </DietDiv2>
                <DietDiv2>
                    <Label htmlFor="glutenFree">Gluten-free</Label>
                    <InputDiet required id="glutenFree"  type="checkbox" onChange={(e) => {handleChange(e.target.id, !formData.glutenFree)}}></InputDiet>
                </DietDiv2>
            </DietDiv>
                <SummaryDiv>
                <Label htmlFor="summary">Summary: </Label>
                <Summary required id="summary" onChange={(e) => {handleChange(e.target.id, e.target.value)}}
                placeholder="Write a summary for your recipe!"
                ></Summary>
                </SummaryDiv>
            <InputDiv>
                <Label htmlFor="servingNumber">Serving number:</Label>
                <Input required id="servingNumber"  type="number" onChange={(e) => {handleChange(e.target.id, e.target.value)}}></Input>
            </InputDiv>
            <InputDiv>
                ...
                {            
                    ...
                    <FormInstructions onSubmit={(e) => {handleAddStep(e)}}>
                        ...
                        <InputDiv>
                            ...
                            value={step.stepNumber} type="number" onChange={(e) => handleStepChange(e.target.id, e.target.value)}></Input>
                        </InputDiv>
                        <InputDiv>
                            <Label htmlFor="equipment">Equipment: </Label>
                            <Input required id="equipment" 
                            value={step.equipment}
                            type="text" onChange={(e) => handleStepChange(e.target.id, e.target.value)}></Input>
                        </InputDiv>
                        <InputDiv>
                            <Label htmlFor="step">Step: </Label>
                            <RecipeInstruction required id="step" type="text" 
                            value={step.step}
                            onChange={(e) => handleStepChange(e.target.id, e.target.value)}></RecipeInstruction>
                        </InputDiv>
                        </div>
                ...
                </FormInstructions>
                {
                    ...
                    }
                    {
                        missingInfo ? 
                        <MissingInfo>
                            <h2>Please fill out all the fields and/or save your ingredients/instructions before creating!</h2>
                        </MissingInfo>
                        :
                        <p></p>
                    }
                    <CreateButton onClick={() => {handleCreate()}}>Create Recipe</CreateButton>
        </Wrapper>
        }
        </>
    )
